...
# simulated settings
I0 = 1e4                    # photon counts
geom = 'parallel'           # geometry, parallel or fanflat
n = 512                     # image width
det_width = 1.0             # detector width
numDec = int(n * 1.5)       # DetectorCount
n_split = 2                 # number of split
angles = 512                # number of angles
...
if geom == 'parallel':
    ...
    det_width = 2.0
    pg = astra.create_proj_geom(geom, det_width, numDec, np.linspace(0, 2*np.pi, angles, False), dso, dod)
